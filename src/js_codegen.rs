...
use crate::{
    codegen::CodegenResult, css::{Rule, Selector}, js_component_scoping::ComponentVariableRenamer, utils::find_and_replace_js_identifiers, Attribute, ClassList, Component, Dialect, Element, Id, JSExpression, Node, ReactiveAttribute, ScriptTag, StaticAttribute, ID_COUNTER
};
...
impl Node {
    pub fn full_js_codegen(&self) -> Result<String, String> {
        let mut root_cvr = CVR::new(&"root".to_string());
        let rrm = RRM::new();

        let _type = CodegenType::HTML;

        Ok(self.codegen_js(&_type, &root_cvr, rrm)?)
    }
...
fn reactive_expression(expr: &JSExpression, update_fn: &JSExpression, cvr: &CVR) -> JSExpression {
    ...
    let subscriptions = reactive_deps
        .iter()
        .map(|dep| {
            let not_state_err = throw_rt_error(format!("${dep} can only be used with a state type.").as_str());
            format!("if (({dep}).__STATE !== true) {not_state_err};\n ({dep}).subscribe(() => ({update_fn})({expr}));",)
        }).collect::<Vec<String>>()
        .join("\n");
